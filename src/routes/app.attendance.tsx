...
function AttendancePage() {
  ...
  const [employeeCoords, setEmployeeCoords] = useState<{ latitude: number; longitude: number; accuracy: number } | null>(null);
            ...
            <div className="flex flex-col sm:flex-row gap-4 items-center justify-between mt-6 pt-6 border-t border-indigo-100/50">
              <div className="text-xs text-muted-foreground leading-relaxed">
                {companySettings?.enforceGeofencing && isInsideRadius === false ? (
                  <span className="text-red-600 font-medium">⚠️ Check-in blocked: You are outside the office geofence.</span>
                ) : (
              ...
              <div className="flex gap-4 w-full sm:w-auto shrink-0">
                <Button
                  ...
                  disabled={hasCheckedIn || (companySettings?.enforceGeofencing && isInsideRadius === false) || gpsStatus === 'acquiring'}
          ...
          <CardHeader className="bg-gradient-to-r from-blue-500/5 to-indigo-500/5 pb-4 border-b border-border/40">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-[10px] text-muted-foreground uppercase tracking-wider">{companySettings?.companyName || "GPS Settings"}</p>
              </div>
            </div>
          </CardHeader>
