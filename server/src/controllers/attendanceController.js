...
exports.generateMonthlySummary = async (req, res) => {
  try {
    ...
    const updatedSummaries = await MonthlyAttendanceSummary.find({ month: paddedMonth, year: year.toString() })
      .populate('employeeId', 'name designation');

    return sendSuccess(res, 'Monthly summary generated and stored successfully', { summaries: updatedSummaries });
  } catch (err) {
    console.error(err);
    return sendError(res, 'Server error');
  }
...
exports.getMonthlySummaries = async (req, res) => {
  try {
    ...
    const paddedMonth = month.toString().padStart(2, '0');
    const summaries = await MonthlyAttendanceSummary.find({ month: paddedMonth, year: year.toString() })
      .populate('employeeId', 'name designation');
