...
exports.register = async (req, res) => {
  try {
    ...
    if (!errors.isEmpty()) return sendError(res, 'Validation failed', 400, { errors: errors.array() });

    const { name, email, password, phone, designation, department, role, dob, joiningDate } = req.body;
    ...
    const emp = new Employee({ name, email, password: hashed, phone, designation, department, role, dob, joiningDate });
    await emp.save();
...
exports.login = async (req, res) => {
  try {
    ...
    return sendSuccess(res, 'Logged in', { token, user: { id: user._id, email: user.email, name: user.name, role: user.role } });
  } catch (err) {
    console.error(err);
    return sendError(res, 'Server error');
  }
